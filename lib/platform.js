...
async function createEvent(payload, user) {
  ...
  await appendValues(EVENT_SHEET_NAME, [[
    event.eventId,
    event.eventType,
    event.eventLabel,
    event.location,
...
async function getEventById(eventId) {
  ...
  const events = await readSheetObjects(EVENT_SHEET_NAME);
  const row = events.find((event) => String(event['Event ID']) === String(eventId));

  if (!row) {
    throw createError(404, 'Event not found.');
  }

  return decorateEvent(mapEventRow(row));
}
...
function decorateEvent(event) {
  return {
    ...
    rsvpPath: `/rsvp/${event.eventId}`,
    attendancePath: `/attendance/${event.eventId}`
  };
}
...
async function ensureBaseSheet(sheetName, headers) {
  ...
  const objects = await readSheetObjects(sheetName);

...
async function handleApiRequest({ method, path: requestPath, headers = {}, body, protocol = 'https' }) {
  ...
  if (method === 'POST' && specialFormSubmitRouteMatch) {
    ...
    await appendSpecialFormResponse(event, payload);
...
function buildSpecialFormColumns(form) {
  const columns = [];
  const used = new Set(SPECIAL_FORM_BASE_HEADERS.map((header) => header.toLowerCase()));
...
async function ensureSpecialFormSheet(event) {
  ...
  const desired = SPECIAL_FORM_BASE_HEADERS.concat(
    buildSpecialFormColumns(event.specialForm).map((entry) => entry.column)
  );

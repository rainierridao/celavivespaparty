...
async function getPublicWellnessRaffleState(event) {
  ...
  const prizes = config.prizes.map((prize) => ({
    ...
    effectiveChance: availableWeight && Math.max(0, prize.quantity - (awards[prize.id] || 0)) > 0
      ? Math.round((WELLNESS_RAFFLE_WIN_CHANCE * prize.chance / availableWeight) * 100) / 100
      : 0
  }));
  return {
    enabled: true,
    ...
    losingVisualChance: config.losingVisualChance || (100 - WELLNESS_RAFFLE_WIN_CHANCE),
    prizes,
...
async function getAdminWellnessRaffleState(event) {
  ...
  return {
    ...config,
    winChance: WELLNESS_RAFFLE_WIN_CHANCE,
...
async function spinWellnessRaffle(event, spinToken) {
  ...
  return withWellnessSpinLock(event.eventId, async () => {
    ...
    const wins = crypto.randomInt(1_000_000) < WELLNESS_RAFFLE_WIN_CHANCE * 10_000;
    let prize = null;
    if (wins) {
...
function normalizeWellnessRaffleConfig(value) {
  ...
  const losingVisualChance = source.losingVisualChance !== undefined
    ...
    : 100 - WELLNESS_RAFFLE_WIN_CHANCE;
  ...
  if (!Number.isFinite(losingVisualChance) || losingVisualChance < 1 || losingVisualChance > 100 - WELLNESS_RAFFLE_WIN_CHANCE) {
    throw badRequest(`Better luck visual size must be between 1% and ${100 - WELLNESS_RAFFLE_WIN_CHANCE}%.`);
  }
  ...
  if (prizes.length) {
    ...
    if (Math.abs(totalChance - WELLNESS_RAFFLE_DISTRIBUTION_TOTAL) > 0.001) {
      throw badRequest(`Winning prize shares must total exactly 100% of the ${WELLNESS_RAFFLE_WIN_CHANCE}% winning pool.`);
    }
  }

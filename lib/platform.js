...
// Standard RSVP + attendance events: same public pages, RSVP defaults, and "view" link
const STANDARD_RSVP_EVENT_TYPES = [CELAVIVE_SPA_PARTY_EVENT_TYPE, WELLNESS_WEDNESDAY_EVENT_TYPE];
const EVENT_TYPES = ['OPP', CELAVIVE_SPA_PARTY_EVENT_TYPE, WELLNESS_WEDNESDAY_EVENT_TYPE, BEAUTY_CARAVAN_EVENT_TYPE, CELAVIVE_RAFFLE_EVENT_TYPE, INBODY_EVENT_TYPE, WELLNESS_QUIZ_EVENT_TYPE, SPECIAL_EVENT_TYPE];
const EVENT_SHEET_NAME = 'Events';
const USER_SHEET_NAME = 'Users';
const PASSWORD_RESET_SHEET_NAME = 'Password Reset Tokens';
const GROUP_DELIVERY_USER_SHEET_NAME = 'Group Delivery Users';
const GROUP_DELIVERY_CYCLE_SHEET_NAME = 'Group Delivery Cycles';
...
const CELAVIVE_SURVEY_ACTIVITY_OPTIONS = [
  'Tuesday / Friday Run',
  'Pickle Ball Game',
  'Yoga',
  'Zumba Nights',
...
const CELAVIVE_SURVEY_RESPONSE_HEADERS = [
  'Timestamp',
  'Event ID',
  'Event Type',
  'Event Label',
...
const SPECIAL_FORM_BASE_HEADERS = [
  'Timestamp',
  'Event ID',
  'Event Type',
  'Event Label',
...
const SPECIAL_FORM_IMAGE_HEADERS = [
  'Image ID',
  'Field ID',
  'Image Name',
  'Data URL',
...
const SPECIAL_FORM_MAX_FIELDS = 60;
const SPECIAL_FORM_MAX_OPTIONS = 40;
const SPECIAL_FORM_MAX_IMAGES = 12;
// Data URLs live in a single Firestore string field, so keep each one well under the 1MB cap.
const SPECIAL_FORM_MAX_IMAGE_CHARS = 600 * 1024;
const SPECIAL_FORM_MAX_RATING = 10;
const SPECIAL_FORM_MAX_PHOTO_BLOCKS = 3;
// Each submitted photo lives in its own one-row table, so the response log stays
// light no matter how many photos come in.
const SPECIAL_FORM_MAX_PHOTO_CHARS = 900 * 1024;
const SPECIAL_PHOTO_TABLE_PREFIX = 'Special Photo';
const SPECIAL_FORM_PHOTO_HEADERS = [
  'Photo ID',
  'Event ID',
  'Field ID',
  'Question',
...
// Recipients read these emails in Philippine time, wherever the server happens to run.
const EVENT_TIME_ZONE = process.env.EVENT_TIME_ZONE || 'Asia/Manila';
const SPECIAL_PAYMENT_METHODS = ['GCash', 'Maya', 'Bank Transfer', 'Cash on site'];
const SPECIAL_PAYMENT_STATUSES = ['Pending', 'Paid', 'Cancelled'];
// Payment columns are appended to every paid form's response sheet.
const SPECIAL_PAYMENT_COLUMNS = [
  'Payment Reference',
  'Payment Method',
  'Payment Email',
  'Payment Amount',
...
// A special event can publish a read-only masterlist of its respondents behind an
// unguessable token, optionally letting whoever holds the link mark payments paid.
const DEFAULT_MASTERLIST_SHARE = {
  enabled: false,
  token: '',
  allowMarkPaid: true,
  createdAt: ''
};
const MASTERLIST_HIDDEN_COLUMNS = [
  '__rowNumber',
  'Event ID',
  'Event Type',
  'Event Label',
...
const GROUP_DELIVERY_USER_HEADERS = ['C/O', 'PIN', 'Registered', 'Status'];
const GROUP_DELIVERY_CYCLE_HEADERS = ['Event ID', 'Cycle Number', 'Status', 'Opened At', 'Submitted At'];
const GROUP_DELIVERY_ENTRY_HEADERS = [
  'Timestamp',
  'Event ID',
  'Event Type',
  'Event Label',
...
const EVENT_HEADERS = [
  ...
  'Group Delivery Entry Sheet Name',
  'Celavive Survey Sheet Name',
  'Special Event Name',
  'Special Form JSON',
  'Special Form Sheet Name',
  'Special Form Image Sheet Name',
  'Special Form Accepting',
  'Special Payment JSON',
  'Masterlist Share JSON'
];
...
async function handleApiRequest({ method, path: requestPath, headers = {}, body, protocol = 'https' }) {
  ...
  if (method === 'POST' && route === '/account/test-email') {
    const user = await requireUser(headers);

    if (!isEmailConfigured()) {
      throw badRequest('Email is not set up yet. Add GMAIL_USER and GMAIL_APP_PASSWORD to your .env, then restart the app.');
    }

    try {
      await getMailer().sendMail({
        to: user.emailAddress,
        subject: 'Test email from GeneSysPH Events',
        html: renderTestEmail(user)
      });
    } catch (error) {
      // The raw reason is the whole point of a test send.
      throw badRequest(error.message);
    }

    return response(200, {
      ok: true,
      message: `Test email sent to ${user.emailAddress}. Check your inbox and spam folder.`
    });
  }
  ...
  const celaviveSurveyResponseRouteMatch = route.match(/^\/events\/([^/]+)\/celavive-survey-responses$/);

  if (method === 'GET' && celaviveSurveyResponseRouteMatch) {
    const user = await requireUser(headers);
    const event = await getUserEventById(celaviveSurveyResponseRouteMatch[1], user);

    if (!isCelaviveSpaPartyEvent(event)) {
      throw createError(404, 'Celavive activity survey responses are not available for this event.');
    }

    await ensureBaseSheet(event.celaviveSurveySheetName, CELAVIVE_SURVEY_RESPONSE_HEADERS, {
      force: true
    });
    const responses = await readSheetObjects(event.celaviveSurveySheetName);
  ...
  const specialFormResponsesRouteMatch = route.match(/^\/events\/([^/]+)\/special-form-responses$/);

  if (method === 'GET' && specialFormResponsesRouteMatch) {
    const user = await requireUser(headers);
    const event = await getUserEventById(specialFormResponsesRouteMatch[1], user);

    if (!isSpecialEvent(event)) {
      throw createError(404, 'Form responses are not available for this event.');
    }

    await ensureSpecialFormSheet(event);
    const responses = await readSheetObjects(event.specialFormSheetName);

    return response(200, { ok: true, event, responses });
  }

  const specialFormImagesRouteMatch = route.match(/^\/events\/([^/]+)\/special-form-images$/);

  if (method === 'GET' && specialFormImagesRouteMatch) {
    const user = await requireUser(headers);
    const event = await getUserEventById(specialFormImagesRouteMatch[1], user);

    if (!isSpecialEvent(event)) {
      throw createError(404, 'Form images are not available for this event.');
    }

    return response(200, { ok: true, images: await listSpecialFormImages(event) });
  }

  const specialFormPhotoRouteMatch = route.match(/^\/events\/([^/]+)\/special-form-photos\/([^/]+)$/);

  if (method === 'GET' && specialFormPhotoRouteMatch) {
    const user = await requireUser(headers);
    const event = await getUserEventById(specialFormPhotoRouteMatch[1], user);
    ...
    return response(200, {
      ok: true,
      photo: await getSpecialFormPhoto(event, specialFormPhotoRouteMatch[2])
    });
  }
  ...
  const groupDeliveryUsersRouteMatch = route.match(/^\/events\/([^/]+)\/group-delivery-users$/);

  if (method === 'GET' && groupDeliveryUsersRouteMatch) {
    const user = await requireUser(headers);
    const event = await getUserEventById(groupDeliveryUsersRouteMatch[1], user);

    if (!isGroupDeliveryEvent(event)) {
      throw createError(404, 'Group Delivery users are not available for this event.');
    }

    return response(200, {
      ok: true,
      event,
      users: await listGroupDeliveryUsers()
    });
  }

  const groupDeliveryUserResetRouteMatch = route.match(/^\/events\/([^/]+)\/group-delivery-users\/([^/]+)\/reset-pin$/);

  if (method === 'PATCH' && groupDeliveryUserResetRouteMatch) {
    const user = await requireUser(headers);
    const event = await getUserEventById(groupDeliveryUserResetRouteMatch[1], user);

    if (!isGroupDeliveryEvent(event)) {
      throw createError(404, 'Group Delivery users are not available for this event.');
    }

    await adminResetGroupDeliveryPin(groupDeliveryUserResetRouteMatch[2]);
    return response(200, { ok: true, message: 'PIN reset approved. The C/O can create a new PIN.' });
  }

  const groupDeliveryCyclesRouteMatch = route.match(/^\/events\/([^/]+)\/group-delivery-cycles$/);

  if (method === 'GET' && groupDeliveryCyclesRouteMatch) {
    const user = await requireUser(headers);
    const event = await getUserEventById(groupDeliveryCyclesRouteMatch[1], user);

    if (!isGroupDeliveryEvent(event)) {
      throw createError(404, 'Group Delivery cycles are not available for this event.');
    }

    return response(200, {
      ok: true,
      event,
      cycles: await listGroupDeliveryCycles(event),
      currentCycle: await ensureCurrentGroupDeliveryCycle(event)
    });
  }

  const groupDeliveryCycleSubmitRouteMatch = route.match(/^\/events\/([^/]+)\/group-delivery-cycles\/current\/submit$/);

  if (method === 'PATCH' && groupDeliveryCycleSubmitRouteMatch) {
    const user = await requireUser(headers);
    const event = await getUserEventById(groupDeliveryCycleSubmitRouteMatch[1], user);

    if (!isGroupDeliveryEvent(event)) {
      throw createError(404, 'Group Delivery cycles are not available for this event.');
    }

    const result = await submitCurrentGroupDeliveryCycle(event);
  ...
  const publicSpecialFormRouteMatch = route.match(/^\/public-special-events\/([^/]+)$/);

  if (method === 'GET' && publicSpecialFormRouteMatch) {
    const event = await getEventById(publicSpecialFormRouteMatch[1]);

    if (!isSpecialEvent(event)) {
      throw createError(404, 'Special event form unavailable.');
    }

    return response(200, {
      ok: true,
      event: sanitizePublicEvent(event),
      images: await listSpecialFormImages(event)
    });
  }

  const specialFormSubmitRouteMatch = route.match(/^\/events\/([^/]+)\/special-form$/);

  if (method === 'POST' && specialFormSubmitRouteMatch) {
    const event = await getEventById(specialFormSubmitRouteMatch[1]);

    if (!isSpecialEvent(event)) {
      throw createError(404, 'Special event form unavailable.');
    }

    if (!event.specialFormAccepting) {
      throw createError(409, 'This form is no longer accepting responses.');
    }

    if (!buildSpecialFormColumns(event.specialForm).length) {
      throw createError(409, 'This form has no questions yet.');
    }

    const payload = normalizeSpecialFormSubmission(body, event);
    const saved = await appendSpecialFormResponse(event, payload);

    return response(200, {
      ok: true,
      message: (event.specialForm && event.specialForm.successMessage) || DEFAULT_SPECIAL_FORM.successMessage,
      payment: saved.paymentReference
        ? {
          required: true,
          reference: saved.paymentReference,
          amount: formatPaymentAmount(event.specialPayment),
          methods: event.specialPayment.methods
        }
        : { required: false }
    });
  }

  const specialPaymentRouteMatch = route.match(/^\/events\/([^/]+)\/special-form\/payment$/);

  if (method === 'POST' && specialPaymentRouteMatch) {
    const event = await getEventById(specialPaymentRouteMatch[1]);
    const result = await saveSpecialPaymentDetails(event, body);

    return response(200, { ok: true, ...result });
  }

  const specialPaymentStatusRouteMatch = route.match(/^\/events\/([^/]+)\/special-payments\/([^/]+)$/);

  if (method === 'PATCH' && specialPaymentStatusRouteMatch) {
    const user = await requireUser(headers);
    const event = await getUserEventById(specialPaymentStatusRouteMatch[1], user);
    const shouldMarkPaid = parseBooleanInput(body && body.paid);
    const result = await markSpecialPaymentPaid(event, specialPaymentStatusRouteMatch[2], shouldMarkPaid);

    return response(200, { ok: true, ...result });
  }

  const sharedMasterlistRouteMatch = route.match(/^\/shared-masterlist\/([^/]+)$/);

  if (method === 'GET' && sharedMasterlistRouteMatch) {
    const event = await findEventByMasterlistToken(sharedMasterlistRouteMatch[1]);

    return response(200, { ok: true, masterlist: await buildSharedMasterlist(event) });
  }

  const sharedMasterlistPaymentRouteMatch = route.match(
    /^\/shared-masterlist\/([^/]+)\/payments\/([^/]+)$/
  );

  if (method === 'PATCH' && sharedMasterlistPaymentRouteMatch) {
    const event = await findEventByMasterlistToken(sharedMasterlistPaymentRouteMatch[1]);

    if (!isSpecialPaymentEvent(event) || !getMasterlistShare(event).allowMarkPaid) {
      throw createError(403, 'This shared masterlist is view only.');
    }

    const shouldMarkPaid = parseBooleanInput(body && body.paid);
    const result = await markSpecialPaymentPaid(event, sharedMasterlistPaymentRouteMatch[2], shouldMarkPaid);

    return response(200, { ok: true, ...result });
  }

  const publicGroupDeliveryRouteMatch = route.match(/^\/public-group-delivery\/([^/]+)$/);

  if (method === 'GET' && publicGroupDeliveryRouteMatch) {
    const event = await getEventById(publicGroupDeliveryRouteMatch[1]);

    if (!isGroupDeliveryEvent(event)) {
      throw createError(404, 'Group Delivery page unavailable.');
    }
  ...
  if (method === 'POST' && route === '/group-delivery/auth/start') {
    const result = await startGroupDeliveryAuth(body);
    return response(200, { ok: true, ...result });
  }

  if (method === 'POST' && route === '/group-delivery/auth/register') {
    const result = await registerGroupDeliveryUser(body);
    return response(200, { ok: true, ...result });
  }

  if (method === 'POST' && route === '/group-delivery/auth/login') {
    const result = await loginGroupDeliveryUser(body);
    return response(200, { ok: true, ...result });
  }

  if (method === 'POST' && route === '/group-delivery/auth/forgot-pin') {
    await requestGroupDeliveryPinReset(body);
    return response(200, { ok: true, message: 'Your PIN reset request has been sent to the admin.' });
  }

  const groupDeliveryMyRecordsRouteMatch = route.match(/^\/group-delivery\/([^/]+)\/my-records$/);

  if (method === 'GET' && groupDeliveryMyRecordsRouteMatch) {
    const event = await getEventById(groupDeliveryMyRecordsRouteMatch[1]);
    const coUser = await requireGroupDeliveryUser(headers);
    ...
    return response(200, {
      ok: true,
      event: sanitizePublicEvent(event),
      ...(await getGroupDeliveryUserRecordState(event, coUser))
    });
  }

  if (method === 'POST' && groupDeliveryMyRecordsRouteMatch) {
    const event = await getEventById(groupDeliveryMyRecordsRouteMatch[1]);
    const coUser = await requireGroupDeliveryUser(headers);
    ...
    const entry = await saveGroupDeliveryEntry(event, coUser, body);
    return response(200, {
      ok: true,
      message: 'Group delivery entry saved successfully.',
      entry,
      ...(await getGroupDeliveryUserRecordState(event, coUser))
    });
  }

  const publicRsvpResponsesMatch = route.match(/^\/public-events\/([^/]+)\/rsvp-responses$/);

  if (method === 'GET' && publicRsvpResponsesMatch) {
    const event = await getEventById(publicRsvpResponsesMatch[1]);

    // Only expose RSVP responses publicly for standard RSVP events
    if (!isStandardRsvpEvent(event)) {
  ...
  const celaviveSurveySubmitRouteMatch = route.match(/^\/events\/([^/]+)\/celavive-survey$/);

  if (method === 'POST' && celaviveSurveySubmitRouteMatch) {
    const event = await getEventById(celaviveSurveySubmitRouteMatch[1]);
    const payload = normalizeCelaviveSurveyPayload(body);

    await appendCelaviveSurveyResponse(event, payload);

    return response(200, {
      ok: true,
      message: 'Your activity interest survey has been submitted successfully.'
    });
  }
...
function sanitizePublicEvent(event) {
  ...
  delete sanitized.celaviveSurveySheetName;
  delete sanitized.wellnessRaffleConfig;
  delete sanitized.wellnessRaffleSpinSheetName;
  // The share token is a password in URL form, so it never rides along on a public page.
  delete sanitized.masterlistShare;
  delete sanitized.masterlistSharePath;
  return sanitized;
}

function getDefaultRsvpAccepting(eventType) {
  return isStandardRsvpEvent({ eventType });
}
...
async function sendPasswordRecoveryEmail({ fullName, emailAddress, resetUrl }) {
  ...
  if (isEmailConfigured()) {
    const result = await trySendEmail({
      ...
      html: renderPasswordRecoveryEmail({ fullName, resetUrl }),
  ...
  if (!env.resendApiKey || !env.emailFrom) {
    console.warn(`Password recovery email not sent because no mail sender is configured. Recovery link for ${emailAddress}: ${resetUrl}`);
    return;
  }
...
function renderTestEmail(user) {
  return `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#151823;">
      <h2 style="margin:0 0 6px;">Email is working</h2>
      <p style="margin:0 0 12px;">
        Hello ${escapeHtml(user.fullName || 'there')}, this is a test from your GeneSysPH Events app.
      </p>
      <p style="margin:0;">
        Payment QR codes and payment confirmations will now reach your attendees.
      </p>
    </div>
  `;
}
...
async function createEvent(payload, user) {
  ...
  const celaviveRaffleSheetName = isCelaviveRaffleEvent(payload)
    ...
    : '';
  const celaviveSurveySheetName = isCelaviveSpaPartyEvent(payload)
    ? makeSafeSheetTitle(`${eventLabel} - Activity Survey - ${sheetSuffix}`)
    : '';
  ...
  const wellnessRaffleSpinSheetName = isWellnessQuizEvent(payload)
    ...
    : '';
  const groupDeliveryEntrySheetName = isGroupDeliveryEvent(payload)
    ? makeSafeSheetTitle(`${eventLabel} - Group Delivery - ${sheetSuffix}`)
    : '';
  const specialFormSheetName = isSpecialEvent(payload)
    ? makeSafeSheetTitle(`${eventLabel} - Responses - ${sheetSuffix}`)
    : '';
  const specialFormImageSheetName = isSpecialEvent(payload)
    ? makeSafeSheetTitle(`${eventLabel} - Form Images - ${sheetSuffix}`)
    : '';
  const event = {
    eventId,
    ...
    archivedAt: '',
    updatedAt: timestamp,
    rsvpAccepting: getDefaultRsvpAccepting(payload.eventType),
    rsvpMaxYes: '',
    ...
    groupDeliveryEntrySheetName,
    specialEventName: isSpecialEvent(payload) ? payload.specialEventName : '',
    specialForm: isSpecialEvent(payload) ? payload.specialForm : { ...DEFAULT_SPECIAL_FORM, fields: [] },
    specialFormSheetName,
    specialFormImageSheetName,
    specialFormAccepting: isSpecialEvent(payload),
    specialPayment: { ...DEFAULT_SPECIAL_PAYMENT }
  };

  if (isSpecialEvent(event)) {
    await persistSpecialFormImages(event, event.specialForm);
  }
  ...
  if (isGroupDeliveryEvent(event)) {
    await ensureGroupDeliveryBaseSheets(event);
  }
...
function mapEventRow(row) {
  return {
    ...
    celaviveSurveySheetName: row['Celavive Survey Sheet Name'] || '',
    ...
    groupDeliveryEntrySheetName: row['Group Delivery Entry Sheet Name'] || '',
    specialEventName: row['Special Event Name'] || '',
    specialForm: parseSpecialForm(row['Special Form JSON']),
    specialFormSheetName: row['Special Form Sheet Name'] || '',
    specialFormImageSheetName: row['Special Form Image Sheet Name'] || '',
    specialFormAccepting: normalizeBooleanSetting(row['Special Form Accepting'], true),
    specialPayment: parseSpecialPayment(row['Special Payment JSON']),
    masterlistShare: parseMasterlistShare(row['Masterlist Share JSON']),
...
function decorateEvent(event) {
  ...
  return {
    ...
    celaviveSurveyPath: isCelaviveSpaPartyEvent(event) ? `/celavive-survey/${publicSlug}` : '',
    wellnessQuizPath: isWellnessQuizEvent(event) ? `/wellness-quiz/${publicSlug}` : '',
    groupDeliveryPath: isGroupDeliveryEvent(event) ? `/group-delivery/${publicSlug}` : '',
    specialFormPath: isSpecialEvent(event) ? `/special-event/${publicSlug}` : '',
    masterlistSharePath: buildMasterlistSharePath(event),
    specialEventDisplayName: isSpecialEvent(event)
      ? event.specialEventName || SPECIAL_EVENT_TYPE
      : ''
  };
}
...
async function ensureEventResponseSheets(event) {
  ...
  if (isCelaviveSpaPartyEvent(nextEvent)) {
    await ensureBaseSheet(nextEvent.celaviveSurveySheetName, CELAVIVE_SURVEY_RESPONSE_HEADERS);
  }
  ...
  if (isGroupDeliveryEvent(nextEvent)) {
    await ensureGroupDeliveryBaseSheets(nextEvent);
  }

  if (isSpecialEvent(nextEvent)) {
    await ensureSpecialFormSheet(nextEvent);
  }
...
function ensureEventResponseSheetNames(event) {
  ...
  if (isCelaviveSpaPartyEvent(nextEvent) && !nextEvent.celaviveSurveySheetName) {
    nextEvent.celaviveSurveySheetName = makeSafeSheetTitle(`${nextEvent.eventLabel} - Activity Survey - ${sheetSuffix}`);
    shouldSave = true;
  }
  ...
  if (isGroupDeliveryEvent(nextEvent) && !nextEvent.groupDeliveryEntrySheetName) {
    nextEvent.groupDeliveryEntrySheetName = makeSafeSheetTitle(`${nextEvent.eventLabel} - Group Delivery - ${sheetSuffix}`);
    shouldSave = true;
  }

  if (isSpecialEvent(nextEvent) && !nextEvent.specialFormSheetName) {
    nextEvent.specialFormSheetName = makeSafeSheetTitle(`${nextEvent.eventLabel} - Responses - ${sheetSuffix}`);
    shouldSave = true;
  }

  if (isSpecialEvent(nextEvent) && !nextEvent.specialFormImageSheetName) {
    nextEvent.specialFormImageSheetName = makeSafeSheetTitle(`${nextEvent.eventLabel} - Form Images - ${sheetSuffix}`);
    shouldSave = true;
  }
...
async function updateEvent(eventId, payload, user) {
  ...
  if (payload.action === 'archive') {
    ...
    nextEvent.eventLabel = buildEventLabel(
      nextEvent.eventType,
      nextEvent.location,
      nextEvent.dateTime,
      nextEvent.specialEventName || ''
    );
    nextEvent.status = 'active';
    nextEvent.archivedAt = '';
  ...
  } else if (payload.action === 'special-form') {
    if (!isSpecialEvent(existingEvent)) {
      throw badRequest('The form builder is only available for special events.');
    }

    const renamed = payload.specialEventName !== existingEvent.specialEventName;

    nextEvent.specialEventName = payload.specialEventName;
    nextEvent.specialForm = payload.specialForm;
    nextEvent.specialFormAccepting = payload.specialFormAccepting;
    nextEvent.specialPayment = {
      ...payload.specialPayment,
      qrImageId: payload.specialPayment.qrImageId || (existingEvent.specialPayment || {}).qrImageId || ''
    };
    ...
    await persistSpecialFormImages(nextEvent, nextEvent.specialForm);
    await persistSpecialPaymentQr(nextEvent, nextEvent.specialPayment);
    await ensureSpecialFormSheet(nextEvent);
    message = payload.specialFormAccepting
      ? 'Form saved and open for responses.'
      : 'Form saved. Responses are closed.';
  } else if (payload.action === 'masterlist-share') {
    if (!isSpecialEvent(existingEvent)) {
      throw badRequest('The shared masterlist is only available for special events.');
    }

    const currentShare = getMasterlistShare(existingEvent);

    if (!payload.enabled) {
      // Dropping the token kills the old link for good, so turning sharing back on
      // always hands out a fresh one instead of resurrecting a link already passed around.
      nextEvent.masterlistShare = {
        ...DEFAULT_MASTERLIST_SHARE,
        allowMarkPaid: payload.allowMarkPaid
      };
      message = 'Masterlist link turned off. The old link no longer opens.';
    } else {
      const shouldRegenerate = payload.regenerate || !currentShare.token;

      nextEvent.masterlistShare = {
        ...currentShare,
        enabled: true,
        allowMarkPaid: payload.allowMarkPaid,
        token: shouldRegenerate ? createMasterlistShareToken() : currentShare.token,
        createdAt: shouldRegenerate ? nextEvent.updatedAt : currentShare.createdAt || nextEvent.updatedAt
      };
      message = shouldRegenerate && currentShare.token
        ? 'New masterlist link created. The previous link no longer works.'
        : payload.allowMarkPaid
          ? 'Masterlist link is live. Anyone with it can view and mark payments paid.'
          : 'Masterlist link is live. Anyone with it can view the list.';
    }
  }
...
function serializeEventRow(event) {
  return [
    ...
    isGroupDeliveryEvent(event) ? event.groupDeliveryEntrySheetName : '',
    isCelaviveSpaPartyEvent(event) ? event.celaviveSurveySheetName : '',
    isSpecialEvent(event) ? event.specialEventName || '' : '',
    isSpecialEvent(event) ? JSON.stringify(event.specialForm || DEFAULT_SPECIAL_FORM) : '',
    isSpecialEvent(event) ? event.specialFormSheetName || '' : '',
    isSpecialEvent(event) ? event.specialFormImageSheetName || '' : '',
    isSpecialEvent(event) && event.specialFormAccepting ? 'TRUE' : 'FALSE',
    isSpecialEvent(event) ? JSON.stringify(event.specialPayment || DEFAULT_SPECIAL_PAYMENT) : '',
    isSpecialEvent(event) ? JSON.stringify(getMasterlistShare(event)) : ''
  ];
}
...
function isCelaviveSpaPartyEvent(event) {
  return String(event && event.eventType ? event.eventType : '').trim() === CELAVIVE_SPA_PARTY_EVENT_TYPE;
}
...
function isGroupDeliveryEvent(event) {
  return String(event && event.eventType ? event.eventType : '').trim() === GROUP_DELIVERY_EVENT_TYPE;
}

function isStandardRsvpEvent(event) {
  return STANDARD_RSVP_EVENT_TYPES.includes(String(event && event.eventType ? event.eventType : '').trim());
}

function isSpecialEvent(event) {
  return String(event && event.eventType ? event.eventType : '').trim() === SPECIAL_EVENT_TYPE;
}

function isSpecialFormDisplayField(field) {
  return SPECIAL_FORM_DISPLAY_TYPES.includes(String(field && field.type ? field.type : '').trim());
}

function isSpecialFormChoiceField(field) {
  return SPECIAL_FORM_CHOICE_TYPES.includes(String(field && field.type ? field.type : '').trim());
}

function createSpecialFieldId() {
  return `fld_${crypto.randomBytes(5).toString('hex')}`;
}

function createSpecialImageId() {
  return `img_${crypto.randomBytes(8).toString('hex')}`;
}

function createSpecialPhotoId() {
  return `pho_${crypto.randomBytes(10).toString('hex')}`;
}

function buildSpecialPhotoTableName(photoId) {
  return makeSafeSheetTitle(`${SPECIAL_PHOTO_TABLE_PREFIX} ${photoId}`);
}
...
function normalizeSpecialEventName(value) {
  const name = String(value === undefined || value === null ? '' : value).trim().replace(/\s+/g, ' ');

  if (!name) {
    throw badRequest('Give this special event a name, for example GeneSys Circle.');
  }

  if (name.length > 80) {
    throw badRequest('The special event name must be 80 characters or fewer.');
  }

  return name;
}

function normalizeSpecialFormDefinition(value) {
  const source = value && typeof value === 'object' ? value : {};
  const rawFields = Array.isArray(source.fields) ? source.fields : [];

  if (rawFields.length > SPECIAL_FORM_MAX_FIELDS) {
    throw badRequest(`A form can hold at most ${SPECIAL_FORM_MAX_FIELDS} blocks.`);
  }

  const fields = rawFields.map((field, index) => normalizeSpecialFormField(field, index)).filter(Boolean);
  const usedIds = new Set();

  for (const field of fields) {
    while (usedIds.has(field.fieldId)) {
      field.fieldId = createSpecialFieldId();
    }

    usedIds.add(field.fieldId);
  }
  ...
  for (let index = 0; index < fields.length; index += 1) {
    ...
    const parent = fields.find((item) => item.fieldId === rule.fieldId);
    const describe = field.label || `Block ${index + 1}`;

    if (!parent || !seenIds.has(rule.fieldId)) {
      // Pointing at itself or at a later block would be unanswerable.
      throw badRequest(`"${describe}" can only depend on a question that comes before it.`);
    }

    if (!isSpecialFormChoiceField(parent)) {
      throw badRequest(`"${describe}" can only depend on a multiple choice, checkbox, dropdown, or poll question.`);
    }

    if (!rule.values.length) {
      throw badRequest(`Choose which answers to "${parent.label}" should reveal "${describe}".`);
    }

    const unknown = rule.values.find((value) => !(parent.options || []).includes(value));

    if (unknown) {
      throw badRequest(`"${describe}" depends on the answer "${unknown}", which "${parent.label}" no longer offers.`);
    }
  ...
  if (imageCount > SPECIAL_FORM_MAX_IMAGES) {
    throw badRequest(`A form can hold at most ${SPECIAL_FORM_MAX_IMAGES} images.`);
  }

  const photoCount = fields.filter((field) => field.type === 'photo-upload').length;

  if (photoCount > SPECIAL_FORM_MAX_PHOTO_BLOCKS) {
    throw badRequest(`A form can ask for at most ${SPECIAL_FORM_MAX_PHOTO_BLOCKS} photo uploads.`);
  }

  const definition = {
    description: String(source.description || '').trim().slice(0, 600),
    submitLabel: String(source.submitLabel || '').trim().slice(0, 40) || DEFAULT_SPECIAL_FORM.submitLabel,
    successMessage:
      String(source.successMessage || '').trim().slice(0, 240) || DEFAULT_SPECIAL_FORM.successMessage,
    headerImageId: String(source.headerImageId || '').trim().slice(0, 60),
    fields
  };

  const headerImageDataUrl = String(source.headerImageDataUrl || '').trim();

  if (headerImageDataUrl) {
    assertValidSpecialImageDataUrl(headerImageDataUrl, 0);
    definition.headerImageDataUrl = headerImageDataUrl;
  }

  return definition;
}

function normalizeSpecialFormField(raw, index) {
  const source = raw && typeof raw === 'object' ? raw : {};
  const type = String(source.type || '').trim();

  if (!SPECIAL_FORM_FIELD_TYPES.includes(type)) {
    // Naming the type makes a stale server obvious instead of looking like a form bug.
    throw badRequest(
      `Block ${index + 1} uses an unsupported type${type ? ` ("${type}")` : ''}. If you just updated the app, restart the server and reload the page.`
    );
  }

  const label = String(source.label || '').trim().replace(/\s+/g, ' ').slice(0, 180);

  if (type !== 'image' && !label) {
    throw badRequest(`Block ${index + 1} needs a question or heading.`);
  }

  const field = {
    fieldId: String(source.fieldId || '').trim().slice(0, 40) || createSpecialFieldId(),
    type,
    label,
    help: String(source.help || '').trim().slice(0, 300),
    required: type === 'image' || type === 'heading' ? false : parseBooleanInput(source.required)
  };

  const showIfSource = source.showIf && typeof source.showIf === 'object' ? source.showIf : null;
  const showIfFieldId = showIfSource ? String(showIfSource.fieldId || '').trim().slice(0, 40) : '';

  if (showIfFieldId) {
    const values = (Array.isArray(showIfSource.values) ? showIfSource.values : [])
      .map((value) => String(value === undefined || value === null ? '' : value).trim())
      .filter(Boolean);

    field.showIf = { fieldId: showIfFieldId, values: values.filter((v, i) => values.indexOf(v) === i) };
  }

  if (isSpecialFormChoiceField(field)) {
    const options = (Array.isArray(source.options) ? source.options : [])
      .map((option) => String(option === undefined || option === null ? '' : option).trim())
      .filter(Boolean);
    const uniqueOptions = [];

    for (const option of options) {
      if (!uniqueOptions.some((existing) => existing.toLowerCase() === option.toLowerCase())) {
        uniqueOptions.push(option.slice(0, 160));
      }
    }

    if (uniqueOptions.length < 2) {
      throw badRequest(`"${label}" needs at least two choices.`);
    }

    if (uniqueOptions.length > SPECIAL_FORM_MAX_OPTIONS) {
      throw badRequest(`"${label}" can hold at most ${SPECIAL_FORM_MAX_OPTIONS} choices.`);
    }

    field.options = uniqueOptions;
    field.allowOther = field.type === 'checkbox' || field.type === 'multiple-choice'
      ? parseBooleanInput(source.allowOther)
      : false;
  }

  if (field.type === 'rating') {
    const ratingMax = Number.parseInt(String(source.ratingMax || SPECIAL_FORM_MAX_RATING), 10);
    field.ratingMax = Number.isInteger(ratingMax) && ratingMax >= 2 && ratingMax <= SPECIAL_FORM_MAX_RATING
      ? ratingMax
      : SPECIAL_FORM_MAX_RATING;
  }

  if (field.type === 'image') {
    const imageDataUrl = String(source.imageDataUrl || '').trim();
    field.imageId = String(source.imageId || '').trim().slice(0, 60);
    field.imageName = String(source.imageName || '').trim().slice(0, 120);

    if (imageDataUrl) {
      assertValidSpecialImageDataUrl(imageDataUrl, index);
      field.imageDataUrl = imageDataUrl;
    }

    if (!field.imageDataUrl && !field.imageId) {
      throw badRequest(`Image block ${index + 1} has no picture uploaded yet.`);
    }
  }
...
function assertValidSpecialImageDataUrl(dataUrl, index) {
  if (!/^data:image\/(png|jpeg|jpg|gif|webp);base64,[A-Za-z0-9+/=]+$/.test(dataUrl)) {
    throw badRequest(`Image block ${index + 1} must be a PNG, JPG, GIF, or WebP picture.`);
  }

  if (dataUrl.length > SPECIAL_FORM_MAX_IMAGE_CHARS) {
    throw badRequest(
      `Image block ${index + 1} is too large. Use a picture under ${Math.floor(SPECIAL_FORM_MAX_IMAGE_CHARS / 1024 * 0.75)}KB.`
    );
  }
}

function parseSpecialForm(value) {
  if (!value) {
    return { ...DEFAULT_SPECIAL_FORM, fields: [] };
  }

  if (typeof value === 'object') {
    return {
      ...DEFAULT_SPECIAL_FORM,
      ...value,
...
// Response columns are named after the visible question so the exported sheet stays readable.
// Duplicate questions are suffixed rather than merged, otherwise two answers would fight over one cell.
function buildSpecialFormColumns(form) {
  const columns = [];
  const used = new Set(
    SPECIAL_FORM_BASE_HEADERS.concat(SPECIAL_PAYMENT_COLUMNS).map((header) => header.toLowerCase())
  );

  for (const field of (form && form.fields) || []) {
    if (isSpecialFormDisplayField(field)) {
      continue;
    }

    const base = String(field.label || 'Question').trim().slice(0, 90) || 'Question';
    let column = base;
    let suffix = 2;

    while (used.has(column.toLowerCase())) {
      column = `${base} (${suffix})`;
      suffix += 1;
    }

    used.add(column.toLowerCase());
    columns.push({ fieldId: field.fieldId, column, field });
  }

  return columns;
}

// Headers only ever grow: renaming or deleting a question must not shift the columns
// that already-saved responses were written into.
async function ensureSpecialFormSheet(event) {
  if (!isSpecialEvent(event) || !event.specialFormSheetName) {
    return [];
  }

  const desired = SPECIAL_FORM_BASE_HEADERS
    .concat(buildSpecialFormColumns(event.specialForm).map((entry) => entry.column))
    .concat(isSpecialPaymentEvent(event) ? SPECIAL_PAYMENT_COLUMNS : []);
  const existingRows = await readSheetRows(event.specialFormSheetName).catch(() => []);
  const existingHeaders = existingRows.length
    ? existingRows[0].map((value) => String(value || '').trim()).filter(Boolean)
    : [];
  ...
  await ensureBaseSheet(event.specialFormSheetName, headers, { force: true });
  // The cached copy of this table still carries the old header row.
  invalidateSheetDataCache(event.specialFormSheetName);

  return headers;
}

async function persistSpecialFormImages(event, form) {
  const pendingFields = (form.fields || []).filter((field) => field.type === 'image' && field.imageDataUrl);
  const hasHeaderUpload = Boolean(form.headerImageDataUrl);

  if (!pendingFields.length && !hasHeaderUpload) {
    (form.fields || []).forEach((field) => {
      delete field.imageDataUrl;
    });
    return form;
  }

  await ensureBaseSheet(event.specialFormImageSheetName, SPECIAL_FORM_IMAGE_HEADERS);

  const timestamp = new Date().toISOString();
  const rows = pendingFields.map((field) => {
    field.imageId = createSpecialImageId();
    return [
      field.imageId,
      field.fieldId,
      field.imageName || '',
  ...
  if (hasHeaderUpload) {
    form.headerImageId = createSpecialImageId();
    rows.push([form.headerImageId, 'form-header', 'header', form.headerImageDataUrl, timestamp]);
  }

  await appendValues(event.specialFormImageSheetName, rows);

  delete form.headerImageDataUrl;
  (form.fields || []).forEach((field) => {
    delete field.imageDataUrl;
  });
...
async function listSpecialFormImages(event) {
  if (!isSpecialEvent(event) || !event.specialFormImageSheetName) {
    return {};
  }

  const referencedIds = new Set(
    ((event.specialForm && event.specialForm.fields) || [])
      .filter((field) => field.type === 'image' && field.imageId)
      .map((field) => field.imageId)
  );

  if (event.specialForm && event.specialForm.headerImageId) {
    referencedIds.add(event.specialForm.headerImageId);
  }

  if (event.specialPayment && event.specialPayment.qrImageId) {
    referencedIds.add(event.specialPayment.qrImageId);
  }
  ...
  const rows = await readSheetObjects(event.specialFormImageSheetName).catch(() => []);
  const images = {};

  for (const row of rows) {
    const imageId = String(row['Image ID'] || '').trim();

    if (imageId && referencedIds.has(imageId)) {
      images[imageId] = String(row['Data URL'] || '');
    }
  }
...
function normalizeSpecialPaymentSettings(value) {
  ...
  const settings = {
    ...
    accountNumber: String(source.accountNumber || '').trim().slice(0, 60),
    instructions: String(source.instructions || '').trim().slice(0, 600),
    qrImageId: String(source.qrImageId || '').trim().slice(0, 60)
  };

  const qrDataUrl = String(source.qrDataUrl || '').trim();

  if (qrDataUrl) {
    assertValidSpecialImageDataUrl(qrDataUrl, 0);
    settings.qrDataUrl = qrDataUrl;
  }
  ...
  if (!rawAmount || !/^\d+(\.\d{1,2})?$/.test(rawAmount) || Number(rawAmount) <= 0) {
    throw badRequest('Enter the payment amount as a number, for example 500 or 500.00.');
  }

  if (!settings.methods.length) {
    throw badRequest('Choose at least one payment method.');
  }

  const needsQr = settings.methods.some((method) => method !== 'Cash on site');

  if (needsQr && !settings.qrDataUrl && !settings.qrImageId) {
    throw badRequest('Upload your payment QR code before turning payments on.');
  }

  return settings;
}

function parseSpecialPayment(value) {
  if (!value) {
    return { ...DEFAULT_SPECIAL_PAYMENT };
  }

  const source = typeof value === 'object' ? value : safeJsonParse(value);
  ...
  return {
    ...
    ...source,
    methods: Array.isArray(source.methods) && source.methods.length
      ? source.methods
      : DEFAULT_SPECIAL_PAYMENT.methods
  };
}
...
function isSpecialPaymentEvent(event) {
  return Boolean(isSpecialEvent(event) && event.specialPayment && event.specialPayment.enabled);
}

function parseMasterlistShare(value) {
  if (!value) {
    return { ...DEFAULT_MASTERLIST_SHARE };
  }
  ...
  return {
    ...
    ...source,
    token,
    enabled: Boolean(source.enabled) && Boolean(token),
    allowMarkPaid: source.allowMarkPaid === undefined ? true : Boolean(source.allowMarkPaid),
    createdAt: String(source.createdAt || '')
  };
}

function getMasterlistShare(event) {
  return parseMasterlistShare(event && event.masterlistShare);
}

function createMasterlistShareToken() {
  return crypto.randomBytes(16).toString('hex');
}

function buildMasterlistSharePath(event) {
  const share = getMasterlistShare(event);

  if (!isSpecialEvent(event) || !share.enabled || !share.token) {
    return '';
  }

  return `/masterlist/${share.token}`;
}

// The share token is the only credential a masterlist link carries, so it is looked
// up across every event rather than scoped to one owner.
async function findEventByMasterlistToken(token) {
  const normalizedToken = String(token || '').trim().toLowerCase();

  if (!/^[a-f0-9]{32}$/.test(normalizedToken)) {
    throw createError(404, 'This shared masterlist link is not valid.');
  }

  ensureGoogleSheetsConfigured();
  await ensureBaseSheet(EVENT_SHEET_NAME, EVENT_HEADERS);

  const rows = await readSheetObjects(EVENT_SHEET_NAME, { cacheTtlMs: CACHE_TTLS.events });
  const match = rows
    .map((row) => decorateEvent(mapEventRow(row)))
    .find((event) => {
      const share = getMasterlistShare(event);
      return share.enabled && share.token.toLowerCase() === normalizedToken;
    });

  // An archived event, or one whose form is closed, keeps its masterlist live. A deleted one does not.
  if (!match || match.isDeleted || !isSpecialEvent(match)) {
    throw createError(404, 'This shared masterlist link is no longer available.');
  }

  return ensureEventResponseSheets(match);
}

async function buildSharedMasterlist(event) {
  await ensureSpecialFormSheet(event);

  const responses = await readSheetObjects(event.specialFormSheetName);
  const share = getMasterlistShare(event);
  const paymentEnabled = isSpecialPaymentEvent(event);
  const columns = buildSharedMasterlistColumns(responses, paymentEnabled);
  const rows = responses.map((row) => {
    const entry = {};

    columns.forEach((column) => {
      entry[column] = String(row[column] === undefined || row[column] === null ? '' : row[column]);
    });

    entry['Payment Reference'] = String(row['Payment Reference'] || '').trim();
    entry['Payment Status'] = String(row['Payment Status'] || '').trim();

    return entry;
  });
  const paidCount = rows.filter((row) => row['Payment Status'] === 'Paid').length;

  return {
    eventName: event.specialEventName || event.eventLabel,
    eventLabel: event.eventLabel,
    location: event.location,
    displayDateTime: event.displayDateTime || event.dateTime,
    acceptingResponses: Boolean(event.specialFormAccepting),
    paymentEnabled,
    paymentAmount: paymentEnabled ? formatPaymentAmount(event.specialPayment) : '',
    allowMarkPaid: Boolean(paymentEnabled && share.allowMarkPaid),
    columns,
    rows,
    totals: {
      responses: rows.length,
      paid: paidCount,
      unpaid: paymentEnabled ? rows.length - paidCount : 0
    }
  };
}

function buildSharedMasterlistColumns(responses, paymentEnabled) {
  const sourceColumns = responses.length ? Object.keys(responses[0]) : [];
  const visibleColumns = sourceColumns.filter((column) => !MASTERLIST_HIDDEN_COLUMNS.includes(column));

  if (paymentEnabled) {
    return visibleColumns;
  }

  return visibleColumns.filter((column) => !SPECIAL_PAYMENT_COLUMNS.includes(column));
}

// The payment QR reuses the per-event image table the display images live in.
async function persistSpecialPaymentQr(event, payment) {
  if (!payment.qrDataUrl) {
    delete payment.qrDataUrl;
    return payment;
  }

  await ensureBaseSheet(event.specialFormImageSheetName, SPECIAL_FORM_IMAGE_HEADERS);

  const imageId = createSpecialImageId();
  await appendValues(event.specialFormImageSheetName, [[
    imageId,
    'payment-qr',
    'payment-qr',
    payment.qrDataUrl,
  ...
  payment.qrImageId = imageId;
  delete payment.qrDataUrl;

  return payment;
}

async function getSpecialPaymentQrDataUrl(event) {
  const imageId = event.specialPayment && event.specialPayment.qrImageId;

  if (!imageId || !event.specialFormImageSheetName) {
    return '';
  }

  const rows = await readSheetObjects(event.specialFormImageSheetName).catch(() => []);
  const record = rows.find((row) => String(row['Image ID'] || '').trim() === imageId);

  return record ? String(record['Data URL'] || '') : '';
}

function createPaymentReference() {
  // Short, readable, and safe to type into a GCash message field.
...
async function findSpecialPaymentRow(event, reference) {
  const code = String(reference || '').trim().toUpperCase();

  if (!/^[A-Z0-9]{4}-[A-Z0-9]{4}$/.test(code)) {
    throw createError(404, 'Payment reference not found.');
  }

  await ensureSpecialFormSheet(event);
  const responses = await readSheetObjects(event.specialFormSheetName);
  const row = responses.find(
    (entry) => String(entry['Payment Reference'] || '').trim().toUpperCase() === code
  );
...
async function updateSpecialFormRow(event, row, updates) {
  const rows = await readSheetRows(event.specialFormSheetName);
  const headers = rows.length ? rows[0].map((value) => String(value || '').trim()) : [];
  const rowNumber = Number(row.__rowNumber || 0);

  if (!headers.length || !rowNumber) {
    throw createError(500, 'That response row could not be updated.');
  }

  const values = headers.map((header) => {
    if (header in updates) {
      return String(updates[header] === undefined || updates[header] === null ? '' : updates[header]);
    }

    return String(row[header] === undefined || row[header] === null ? '' : row[header]);
  });

  await updateRange(
    `${event.specialFormSheetName}!A${rowNumber}:${columnLetter(headers.length)}${rowNumber}`,
    [values]
  );
...
// Step two of a paid submission: the payer says how they will pay and where to
// send the QR. Re-running it simply re-sends, which covers a mistyped address.
async function saveSpecialPaymentDetails(event, body) {
  if (!isSpecialPaymentEvent(event)) {
    throw createError(404, 'This form does not collect payments.');
  }

  const payment = event.specialPayment;
  const reference = String(body && body.reference ? body.reference : '').trim().toUpperCase();
  const emailAddress = String(body && body.emailAddress ? body.emailAddress : '').trim();
  const method = String(body && body.method ? body.method : '').trim();
  ...
  if (!isValidEmailAddress(emailAddress)) {
    throw badRequest('Enter a valid email address so we can send your payment QR.');
  }

  const row = await findSpecialPaymentRow(event, reference);

  if (String(row['Payment Status'] || '').trim() === 'Paid') {
    throw createError(409, 'This payment has already been confirmed.');
  }

  await updateSpecialFormRow(event, row, {
    'Payment Method': method,
    'Payment Email': emailAddress
  });

  const qrDataUrl = method === 'Cash on site' ? '' : await getSpecialPaymentQrDataUrl(event);
  const emailResult = await trySendEmail({
    to: emailAddress,
    subject: `Payment instructions for ${event.specialEventName || event.eventLabel} (${reference})`,
    html: renderSpecialPaymentEmail({ event, payment, reference, method, qrDataUrl }),
    attachments: qrDataUrl
      ? [{ cid: 'paymentqr', dataUrl: qrDataUrl, fileName: 'payment-qr.png' }]
      : [],
    context: 'special-payment-qr'
  });
  ...
  return {
    ...
    emailMessage: emailResult.sent
      ? `We also sent this to ${emailAddress}.`
      : 'Email is not set up yet, so please screenshot this page.'
  };
}

// Confirmation is a deliberate act by the organiser: GCash cannot tell this app
// that money arrived, so nothing here is automatic.
async function markSpecialPaymentPaid(event, reference, shouldMarkPaid) {
  if (!isSpecialEvent(event)) {
    throw badRequest('Payments are only available for special events.');
  }

  const row = await findSpecialPaymentRow(event, reference);
  const nextStatus = shouldMarkPaid ? 'Paid' : 'Pending';
  const currentStatus = String(row['Payment Status'] || '').trim();

  if (currentStatus === nextStatus) {
    return {
      message: `Payment is already marked ${nextStatus.toLowerCase()}.`,
      tone: 'warning',
      emailSent: false
    };
  }

  const updatedRow = await updateSpecialFormRow(event, row, {
    'Payment Status': nextStatus,
    'Paid At': shouldMarkPaid ? new Date().toISOString() : ''
  });

  if (!shouldMarkPaid) {
    return { message: 'Payment moved back to pending.', tone: 'success', emailSent: false };
  }

  const emailAddress = String(row['Payment Email'] || '').trim();

  if (!isValidEmailAddress(emailAddress)) {
    return {
      message: 'Marked as paid. No confirmation email was sent because this entry has no email address.',
      tone: 'warning',
      emailSent: false
    };
  }
  ...
  const emailResult = await trySendEmail({
    to: emailAddress,
    subject: `Payment confirmed for ${event.specialEventName || event.eventLabel} (${row['Payment Reference']})`,
    html: renderSpecialPaymentConfirmationEmail({ event, row: updatedRow }),
    context: 'special-payment-confirmation'
  });

  return {
    message: emailResult.sent
      ? `Marked as paid. Confirmation sent to ${emailAddress}.`
      : `Marked as paid, but the confirmation email to ${emailAddress} could not be sent. ${
        emailResult.reason === 'not-configured'
          ? 'Email is not set up yet: add GMAIL_USER and GMAIL_APP_PASSWORD, then restart the app.'
          : emailResult.message || 'Check your email settings.'
      }`,
    tone: emailResult.sent ? 'success' : 'error',
    emailSent: emailResult.sent
  };
}
...
function renderSpecialPaymentEmail({ event, payment, reference, method, qrDataUrl }) {
  const title = escapeHtml(event.specialEventName || event.eventLabel);
  const amount = escapeHtml(formatPaymentAmount(payment));
  const rows = [
    ['Event', title],
    ['Amount', amount],
    ['Payment method', escapeHtml(method)],
    ['Reference code', escapeHtml(reference)]
  ];

  if (payment.accountName) {
    rows.push(['Account name', escapeHtml(payment.accountName)]);
  }

  if (payment.accountNumber) {
    rows.push(['Account number', escapeHtml(payment.accountNumber)]);
  }

  return `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#151823;max-width:520px;">
      <h2 style="margin:0 0 6px;">Almost done</h2>
      <p style="margin:0 0 16px;">Here are your payment details for <strong>${title}</strong>.</p>
      <table style="border-collapse:collapse;margin-bottom:18px;">
        ${rows
          .map(
            ([label, value]) => `
              <tr>
                <td style="padding:5px 16px 5px 0;color:#6b7280;">${label}</td>
                <td style="padding:5px 0;font-weight:700;">${value}</td>
              </tr>
            `
          )
          .join('')}
      ...
      ${
        qrDataUrl
          ? `
            <p style="margin:0 0 10px;font-weight:700;">Scan this QR to pay:</p>
            <img src="cid:paymentqr" alt="Payment QR code" style="width:260px;max-width:100%;border:1px solid #e5e7eb;border-radius:12px;">
          `
          : '<p style="margin:0 0 10px;">Please pay on site when you arrive.</p>'
      }
      <p style="margin:18px 0 0;">
        Please put your reference code <strong>${escapeHtml(reference)}</strong> in the payment notes
        so we can match it to your entry.
      </p>
      <p style="margin:14px 0 0;color:#6b7280;font-size:13px;">
        You will get a confirmation email once your payment has been checked and approved.
      </p>
    </div>
  `;
}

// 'Paid At' is a true UTC instant, unlike the naive date strings elsewhere in this
// file. Without an explicit zone it renders in the SERVER's timezone, which is UTC
// on Netlify and put every confirmation 8 hours behind local time.
// Seconds matter too: confirming twice in the same minute would otherwise produce
// byte-identical emails, which mail clients collapse into one.
function formatConfirmationTimestamp(value) {
...
function renderSpecialPaymentConfirmationEmail({ event, row }) {
  const title = escapeHtml(event.specialEventName || event.eventLabel);
  ...
  return `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#151823;max-width:520px;">
      ...
      <p style="margin:0 0 16px;">
        Thank you! We have received your payment for <strong>${title}</strong>.
      </p>
      <table style="border-collapse:collapse;margin-bottom:18px;">
        <tr>
          <td style="padding:5px 16px 5px 0;color:#6b7280;">Reference code</td>
          <td style="padding:5px 0;font-weight:700;">${escapeHtml(row['Payment Reference'] || '')}</td>
        </tr>
        <tr>
          <td style="padding:5px 16px 5px 0;color:#6b7280;">Amount</td>
          <td style="padding:5px 0;font-weight:700;">${escapeHtml(row['Payment Amount'] || '')}</td>
        </tr>
        <tr>
          <td style="padding:5px 16px 5px 0;color:#6b7280;">Method</td>
          <td style="padding:5px 0;font-weight:700;">${escapeHtml(row['Payment Method'] || '')}</td>
        </tr>
        <tr>
          <td style="padding:5px 16px 5px 0;color:#6b7280;">Confirmed on</td>
          <td style="padding:5px 0;font-weight:700;">${escapeHtml(formatConfirmationTimestamp(row['Paid At']))}</td>
        </tr>
      </table>
      <p style="margin:0;">We look forward to seeing you at ${escapeHtml(event.location || 'the event')}.</p>
    </div>
  `;
}

// Works out which blocks a given set of answers actually reveals. A block is
// visible only when its rule matches AND the question it depends on is itself
// visible, so a chain collapses the moment any link stops matching.
function evaluateSpecialFormVisibility(form, rawAnswers) {
  ...
  for (const field of (form && form.fields) || []) {
    ...
    const given = rawAnswers ? rawAnswers[rule.fieldId] : undefined;
    const selected = Array.isArray(given)
      ? given.map((value) => String(value).trim())
      : [String(given === undefined || given === null ? '' : given).trim()];

    if (selected.some((value) => value && rule.values.includes(value))) {
      visible.add(field.fieldId);
    }
  }
...
function normalizeSpecialFormSubmission(body, event) {
  const form = event.specialForm || { fields: [] };
  const rawAnswers = body && typeof body.answers === 'object' && body.answers ? body.answers : {};
  const answers = {};
  const photos = [];
  const visible = evaluateSpecialFormVisibility(form, rawAnswers);

  for (const entry of buildSpecialFormColumns(form)) {
    const field = entry.field;

    // A block the answers never revealed is neither required nor recorded, even
    // if a stale value was posted for it.
    if (!visible.has(field.fieldId)) {
      answers[entry.column] = '';
      continue;
    }

    const raw = rawAnswers[field.fieldId];

    if (field.type === 'photo-upload') {
      const photo = normalizeSpecialPhotoAnswer(field, raw);

      if (field.required && !photo) {
        throw badRequest(`"${field.label}" needs a photo.`);
      }

      answers[entry.column] = '';

      if (photo) {
        photos.push({ ...photo, column: entry.column, fieldId: field.fieldId, question: field.label });
      }

      continue;
    }

    const value = normalizeSpecialFormAnswer(field, raw);

    if (field.required && !value) {
      throw badRequest(`"${field.label}" is required.`);
    }

    answers[entry.column] = value;
  }

  return { answers, photos };
}

function normalizeSpecialPhotoAnswer(field, raw) {
  const source = raw && typeof raw === 'object' ? raw : { dataUrl: raw };
  ...
  if (!/^data:image\/(png|jpeg|jpg|gif|webp);base64,[A-Za-z0-9+/=]+$/.test(dataUrl)) {
    throw badRequest(`"${field.label}" must be a PNG, JPG, GIF, or WebP photo.`);
  }

  if (dataUrl.length > SPECIAL_FORM_MAX_PHOTO_CHARS) {
    throw badRequest(`The photo for "${field.label}" is too large. Please choose a smaller one.`);
  }
...
async function storeSpecialFormPhoto(event, photo) {
  const photoId = createSpecialPhotoId();
  const tableName = buildSpecialPhotoTableName(photoId);

  await ensureBaseSheet(tableName, SPECIAL_FORM_PHOTO_HEADERS);
  await appendValues(tableName, [[
    photoId,
    event.eventId,
    photo.fieldId,
    photo.question || '',
...
async function getSpecialFormPhoto(event, photoId) {
  if (!isSpecialPhotoReference(photoId)) {
    throw createError(404, 'Photo not found.');
  }

  const rows = await readSheetObjects(buildSpecialPhotoTableName(photoId)).catch(() => []);
  const record = rows.find((row) => String(row['Photo ID'] || '').trim() === photoId);

  // A photo is only readable through the event that collected it.
  if (!record || String(record['Event ID'] || '').trim() !== event.eventId) {
    throw createError(404, 'Photo not found.');
  }
  ...
  return {
    ...
    dataUrl: record['Data URL'] || ''
  };
}

function normalizeSpecialFormAnswer(field, raw) {
  if (field.type === 'checkbox') {
    const selected = (Array.isArray(raw) ? raw : raw === undefined || raw === null || raw === '' ? [] : [raw])
      .map((item) => String(item).trim())
      .filter(Boolean);
    const allowed = field.options || [];
    const cleaned = selected.filter(
      (item) => allowed.includes(item) || (field.allowOther && item.length <= 200)
    );

    if (selected.length && !cleaned.length) {
      throw badRequest(`Choose a valid option for "${field.label}".`);
    }
  ...
  if (isSpecialFormChoiceField(field)) {
    ...
    if (!allowed.includes(value) && !field.allowOther) {
      throw badRequest(`Choose a valid option for "${field.label}".`);
    }
  ...
  if (field.type === 'rating') {
    ...
    const max = field.ratingMax || SPECIAL_FORM_MAX_RATING;

    if (!Number.isInteger(rating) || rating < 1 || rating > max) {
      throw badRequest(`Choose a rating between 1 and ${max} for "${field.label}".`);
    }
  ...
  if (field.type === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
    throw badRequest(`Enter a valid email address for "${field.label}".`);
  }

  return value.slice(0, field.type === 'paragraph' ? 4000 : 500);
}

async function appendSpecialFormResponse(event, payload) {
  const headers = await ensureSpecialFormSheet(event);

  for (const photo of payload.photos || []) {
    payload.answers[photo.column] = await storeSpecialFormPhoto(event, photo);
  }

  const paymentReference = isSpecialPaymentEvent(event) ? createPaymentReference() : '';

  if (paymentReference) {
    payload.answers['Payment Reference'] = paymentReference;
    payload.answers['Payment Amount'] = formatPaymentAmount(event.specialPayment);
    payload.answers['Payment Status'] = 'Pending';
  }
  ...
  const row = headers.map((header) =>
    header in baseValues ? baseValues[header] : payload.answers[header] || ''
  );

  await appendValues(event.specialFormSheetName, [row]);
...
async function appendCelaviveSurveyResponse(event, payload) {
  ensureGoogleSheetsConfigured();

  if (!isCelaviveSpaPartyEvent(event)) {
    throw badRequest('This event does not accept Celavive activity survey submissions.');
  }

  await ensureBaseSheet(event.celaviveSurveySheetName, CELAVIVE_SURVEY_RESPONSE_HEADERS);

  await appendValues(event.celaviveSurveySheetName, [[
    new Date().toISOString(),
    event.eventId,
    event.eventType,
    event.eventLabel,
    ...
    payload.invitedBy,
    payload.partyRating,
    payload.futureActivities.join(', '),
    payload.futureActivities.map((activity) => `${activity}: ${payload.activityWillingness[activity]}`).join(', ')
  ]]);
}
...
function getResponseSheetTarget(event, responseType) {
  ...
  if (normalized === 'celavive-survey') {
    if (!isCelaviveSpaPartyEvent(event)) {
      throw badRequest('Celavive activity survey responses are not available for this event.');
    }

    return {
      sheetName: event.celaviveSurveySheetName,
      headers: CELAVIVE_SURVEY_RESPONSE_HEADERS,
      label: 'Celavive activity survey'
    };
  }
  ...
  if (normalized === 'special-form') {
    if (!isSpecialEvent(event)) {
      throw badRequest('Form responses are not available for this event.');
    }

    return {
      sheetName: event.specialFormSheetName,
      headers: SPECIAL_FORM_BASE_HEADERS,
      label: event.specialEventName || 'Special event',
      special: true
    };
  }
...
async function ensureGroupDeliveryBaseSheets(event) {
  await ensureBaseSheet(GROUP_DELIVERY_USER_SHEET_NAME, GROUP_DELIVERY_USER_HEADERS, { force: true });
  await ensureBaseSheet(GROUP_DELIVERY_CYCLE_SHEET_NAME, GROUP_DELIVERY_CYCLE_HEADERS, { force: true });
  await ensureBaseSheet(event.groupDeliveryEntrySheetName, GROUP_DELIVERY_ENTRY_HEADERS, { force: true });
  await ensureCurrentGroupDeliveryCycle(event);
}

async function listGroupDeliveryUsers() {
  ensureGoogleSheetsConfigured();
  await ensureBaseSheet(GROUP_DELIVERY_USER_SHEET_NAME, GROUP_DELIVERY_USER_HEADERS, { force: true });
  const rows = await readSheetObjects(GROUP_DELIVERY_USER_SHEET_NAME);
  return rows.map((row) => ({
    co: row['C/O'] || '',
    registered: parseBooleanInput(row.Registered),
    status: normalizeGroupDeliveryUserStatus(row.Status),
    hasPin: Boolean(row.PIN),
    rowNumber: Number(row.__rowNumber || 0)
  }));
}

async function findGroupDeliveryUser(co) {
  const normalizedCo = normalizeGroupDeliveryCo(co);
  if (!normalizedCo) return null;
  await ensureBaseSheet(GROUP_DELIVERY_USER_SHEET_NAME, GROUP_DELIVERY_USER_HEADERS, { force: true });
  const rows = await readSheetObjects(GROUP_DELIVERY_USER_SHEET_NAME);
  const match = rows.find((row) => normalizeGroupDeliveryCo(row['C/O']) === normalizedCo);
  return match ? mapGroupDeliveryUserRow(match) : null;
}

function mapGroupDeliveryUserRow(row) {
  return {
    co: row['C/O'] || '',
    pinHash: row.PIN || '',
    registered: parseBooleanInput(row.Registered),
...
async function startGroupDeliveryAuth(body) {
  const co = normalizeGroupDeliveryCo(body && body.co);
  if (!co) throw badRequest('C/O is required.');
  const user = await findGroupDeliveryUser(co);

  if (!user || !user.pinHash || !user.registered || user.status === 'reset-approved') {
    return { co, mode: 'register', status: user ? user.status : 'new' };
  }

  if (user.status === 'reset-requested') {
    return { co, mode: 'pending-reset', status: user.status, message: 'Your PIN reset is waiting for admin approval.' };
  }

  return { co, mode: 'login', status: user.status };
}

async function registerGroupDeliveryUser(body) {
  const co = normalizeGroupDeliveryCo(body && body.co);
  const pin = normalizeGroupDeliveryPin(body && body.pin);
  const confirmPin = normalizeGroupDeliveryPin(body && body.confirmPin);

  if (!co) throw badRequest('C/O is required.');
  if (!pin) throw badRequest('PIN must be 4 to 6 digits.');
  if (pin !== confirmPin) throw badRequest('PIN confirmation does not match.');

  const existingUser = await findGroupDeliveryUser(co);
  if (existingUser && existingUser.status === 'reset-requested') {
    throw createError(409, 'Your PIN reset is waiting for admin approval.');
  }
  if (existingUser && existingUser.pinHash && existingUser.registered && existingUser.status === 'active') {
    throw createError(409, 'This C/O already has a PIN. Please log in instead.');
  }

  await assertUniqueGroupDeliveryPin(pin, co);
  const nextUser = {
    'C/O': co,
    PIN: createPasswordHash(pin),
    Registered: 'TRUE',
    Status: 'Active'
  };

  if (existingUser && existingUser.rowNumber) {
    await updateSheetObjectRow(GROUP_DELIVERY_USER_SHEET_NAME, GROUP_DELIVERY_USER_HEADERS, {
      ...nextUser,
      __rowNumber: existingUser.rowNumber
    });
  } else {
    await appendValues(GROUP_DELIVERY_USER_SHEET_NAME, [[nextUser['C/O'], nextUser.PIN, nextUser.Registered, nextUser.Status]]);
  }

  const saved = await findGroupDeliveryUser(co);
  return {
    co,
    token: createGroupDeliverySessionToken(saved),
    user: sanitizeGroupDeliveryUser(saved)
  };
}

async function loginGroupDeliveryUser(body) {
  const co = normalizeGroupDeliveryCo(body && body.co);
  const pin = normalizeGroupDeliveryPin(body && body.pin);
  ...
  if (!user || !user.pinHash || !user.registered) {
    throw createError(401, 'Please create a PIN first.');
  }
  ...
  if (user.status !== 'active' || !verifyPassword(pin, user.pinHash)) {
    throw createError(401, 'Invalid C/O or PIN.');
  }

  return {
    co: user.co,
    token: createGroupDeliverySessionToken(user),
    user: sanitizeGroupDeliveryUser(user)
  };
}

async function requestGroupDeliveryPinReset(body) {
  const co = normalizeGroupDeliveryCo(body && body.co);
  if (!co) throw badRequest('C/O is required.');
  const user = await findGroupDeliveryUser(co);
  if (!user) return;
  await updateSheetObjectRow(GROUP_DELIVERY_USER_SHEET_NAME, GROUP_DELIVERY_USER_HEADERS, {
    'C/O': user.co,
    PIN: user.pinHash,
    Registered: user.registered ? 'TRUE' : 'FALSE',
    Status: 'Reset Requested',
    __rowNumber: user.rowNumber
  });
}

async function adminResetGroupDeliveryPin(encodedCo) {
  const co = normalizeGroupDeliveryCo(decodeURIComponent(String(encodedCo || '')));
  const user = await findGroupDeliveryUser(co);
  if (!user) throw createError(404, 'C/O user not found.');
  await updateSheetObjectRow(GROUP_DELIVERY_USER_SHEET_NAME, GROUP_DELIVERY_USER_HEADERS, {
    'C/O': user.co,
    PIN: '',
    Registered: 'FALSE',
    Status: 'Reset Approved',
    __rowNumber: user.rowNumber
  });
}

async function assertUniqueGroupDeliveryPin(pin, currentCo = '') {
  const users = await listGroupDeliveryUsersWithHashes();
  const current = normalizeGroupDeliveryCo(currentCo);
  const duplicate = users.some((user) => (
    normalizeGroupDeliveryCo(user.co) !== current &&
    user.pinHash &&
    user.registered &&
    user.status === 'active' &&
    verifyPassword(pin, user.pinHash)
  ));

  if (duplicate) {
    throw createError(409, 'That PIN is already in use. Please choose a different PIN.');
  }
}

async function listGroupDeliveryUsersWithHashes() {
  await ensureBaseSheet(GROUP_DELIVERY_USER_SHEET_NAME, GROUP_DELIVERY_USER_HEADERS, { force: true });
  return (await readSheetObjects(GROUP_DELIVERY_USER_SHEET_NAME)).map(mapGroupDeliveryUserRow);
}

async function requireGroupDeliveryUser(headers) {
  const authorization = String(headers.authorization || headers.Authorization || '').trim();
  const token = authorization.toLowerCase().startsWith('bearer ') ? authorization.slice(7).trim() : '';

  if (!token) {
    throw createError(401, 'Please log in with your C/O PIN.');
  }

  const payload = verifySignedToken(token, 'group-delivery');
  const user = await findGroupDeliveryUser(payload.co);

  if (!user || !user.registered || user.status !== 'active' || !user.pinHash) {
    throw createError(401, 'Your C/O session is no longer valid.');
  }

  if (payload.pinFingerprint !== createGroupDeliveryPinFingerprint(user.pinHash)) {
    throw createError(401, 'Your C/O session is no longer valid.');
  }
...
function createGroupDeliverySessionToken(user) {
  return createSignedToken({
    purpose: 'group-delivery',
    co: user.co,
    pinFingerprint: createGroupDeliveryPinFingerprint(user.pinHash),
    exp: Date.now() + 1000 * 60 * 60 * 24 * 30
  });
}

function createGroupDeliveryPinFingerprint(pinHash) {
  return crypto.createHash('sha256').update(String(pinHash || '')).digest('base64url');
}

function sanitizeGroupDeliveryUser(user) {
  return {
    co: user.co,
...
async function ensureCurrentGroupDeliveryCycle(event) {
  await ensureBaseSheet(GROUP_DELIVERY_CYCLE_SHEET_NAME, GROUP_DELIVERY_CYCLE_HEADERS, { force: true });
  const cycles = await listGroupDeliveryCycles(event);
  const current = cycles.find((cycle) => cycle.status === 'open');

  if (current) {
    return current;
  }

  const nextCycleNumber = cycles.reduce((max, cycle) => Math.max(max, Number(cycle.cycleNumber || 0)), 0) + 1;
  const cycle = {
  ...
  await appendValues(GROUP_DELIVERY_CYCLE_SHEET_NAME, [[
    cycle.eventId,
    cycle.cycleNumber,
    'Open',
    cycle.openedAt,
...
async function listGroupDeliveryCycles(event) {
  await ensureBaseSheet(GROUP_DELIVERY_CYCLE_SHEET_NAME, GROUP_DELIVERY_CYCLE_HEADERS, { force: true });
  return (await readSheetObjects(GROUP_DELIVERY_CYCLE_SHEET_NAME))
    .filter((row) => String(row['Event ID'] || '') === event.eventId)
    .map((row) => ({
      eventId: row['Event ID'] || '',
      cycleNumber: Number(row['Cycle Number'] || 0),
      status: normalizeGroupDeliveryCycleStatus(row.Status),
      openedAt: row['Opened At'] || '',
...
async function submitCurrentGroupDeliveryCycle(event) {
  const current = await ensureCurrentGroupDeliveryCycle(event);
  const submittedAt = new Date().toISOString();

  await updateSheetObjectRow(GROUP_DELIVERY_CYCLE_SHEET_NAME, GROUP_DELIVERY_CYCLE_HEADERS, {
    'Event ID': event.eventId,
    'Cycle Number': current.cycleNumber,
    Status: 'Submitted',
    'Opened At': current.openedAt,
    'Submitted At': submittedAt,
    __rowNumber: current.rowNumber
  });
  ...
  return {
    message: `Cycle ${current.cycleNumber} submitted. Cycle ${current.cycleNumber + 1} is now open.`,
    submittedCycle: { ...current, status: 'submitted', submittedAt },
    currentCycle: await ensureCurrentGroupDeliveryCycle(event)
  };
}

async function getGroupDeliveryUserRecordState(event, coUser) {
  await ensureGroupDeliveryBaseSheets(event);
  const currentCycle = await ensureCurrentGroupDeliveryCycle(event);
  const rows = (await readSheetObjects(event.groupDeliveryEntrySheetName))
    .filter((row) => normalizeGroupDeliveryCo(row['C/O']) === normalizeGroupDeliveryCo(coUser.co))
    .sort((left, right) => Number(left['Cycle Number'] || 0) - Number(right['Cycle Number'] || 0));
  const history = rows.filter((row) => Number(row['Cycle Number'] || 0) < currentCycle.cycleNumber || normalizeGroupDeliveryEntryStatus(row['Entry Status']) === 'submitted');
  const currentEntry = rows.find((row) => Number(row['Cycle Number'] || 0) === currentCycle.cycleNumber) || null;

  return {
    currentCycle,
    history: history.map(formatGroupDeliveryEntry),
    currentEntry: currentEntry ? formatGroupDeliveryEntry(currentEntry) : null,
    user: sanitizeGroupDeliveryUser(coUser)
  };
}

async function saveGroupDeliveryEntry(event, coUser, body) {
  await ensureGroupDeliveryBaseSheets(event);
  const currentCycle = await ensureCurrentGroupDeliveryCycle(event);
  const payload = normalizeGroupDeliveryEntryPayload(body);
  const rows = await readSheetObjects(event.groupDeliveryEntrySheetName);
  const existing = rows.find((row) => (
    normalizeGroupDeliveryCo(row['C/O']) === normalizeGroupDeliveryCo(coUser.co) &&
    Number(row['Cycle Number'] || 0) === currentCycle.cycleNumber
  ));
  ...
  const rowObject = {
    ...
    'Date Time': event.dateTime,
    'C/O': coUser.co,
    'Cycle Number': currentCycle.cycleNumber,
    'Entry Status': 'Draft',
    'Submitted At': '',
  ...
  if (existing && existing.__rowNumber) {
    await updateSheetObjectRow(event.groupDeliveryEntrySheetName, GROUP_DELIVERY_ENTRY_HEADERS, {
      ...rowObject,
      __rowNumber: existing.__rowNumber
    });
  } else {
    await appendValues(event.groupDeliveryEntrySheetName, [GROUP_DELIVERY_ENTRY_HEADERS.map((header) => rowObject[header] || '')]);
  }

  return formatGroupDeliveryEntry(rowObject);
}

function formatGroupDeliveryEntry(row) {
  return {
    co: row['C/O'] || '',
    cycleNumber: Number(row['Cycle Number'] || 0),
    entryStatus: normalizeGroupDeliveryEntryStatus(row['Entry Status']),
    submittedAt: row['Submitted At'] || '',
    ...
    updatedAt: row['Updated At'] || row.Timestamp || ''
  };
}
...
function matchesEventIdentifier(row, identifier) {
  ...
  const stripped = normalized.endsWith('view') ? normalized.slice(0, -4) : normalized;

  // Allow a trailing "view" suffix only for standard RSVP public slugs
  const allowViewSuffix = isStandardRsvpEvent(event);

  return (
    event.eventId === normalized ||
    (event.publicSlug || buildPublicEventSlug(event)) === normalized ||
    buildLegacyPublicEventSlug(event) === normalized ||
    (allowViewSuffix && stripped && (
      event.eventId === stripped ||
      (event.publicSlug || buildPublicEventSlug(event)) === stripped ||
      buildLegacyPublicEventSlug(event) === stripped
    ))
  );
}
...
function buildEventLabel(eventType, location, dateTime, displayName = '') {
  const heading = String(displayName || '').trim() || eventType;
  return `${heading} - ${formatDisplayDateTime(dateTime)} - ${location}`;
}
...
function normalizeEventPayload(body) {
  ...
  if (eventType === SPECIAL_EVENT_TYPE) {
    return {
      eventType,
      location,
      dateTime,
      specialEventName: normalizeSpecialEventName(body && body.specialEventName),
      specialForm: normalizeSpecialFormDefinition(body && body.specialForm)
    };
  }
...
function normalizeEventMutationPayload(body) {
  ...
  if (!['archive', 'unarchive', 'reschedule', 'rsvp-settings', 'celavive-entry-settings', 'inbody-settings', 'wellness-raffle-settings', 'special-form', 'masterlist-share'].includes(action)) {
  ...
  if (action === 'reschedule') {
    ...
    return { action, location, dateTime };
  }

  if (action === 'special-form') {
    return {
      action,
      specialEventName: normalizeSpecialEventName(body && body.specialEventName),
      specialForm: normalizeSpecialFormDefinition(body && body.specialForm),
      specialFormAccepting: parseBooleanInput(body && body.specialFormAccepting),
      specialPayment: normalizeSpecialPaymentSettings(body && body.specialPayment)
    };
  }

  if (action === 'masterlist-share') {
    return {
      action,
      enabled: parseBooleanInput(body && body.enabled),
      allowMarkPaid: parseBooleanInput(body && body.allowMarkPaid),
      regenerate: parseBooleanInput(body && body.regenerate)
    };
  }
...
function normalizeRsvpPayload(body, event) {
  const payload = {
    ...
    threadsName: String(body && body.threadsName ? body.threadsName : '').trim(),
...
function normalizeGroupDeliveryEntryPayload(body) {
  return normalizeAttendancePayload(body);
}

function normalizeGroupDeliveryCo(value) {
  return String(value || '').trim().replace(/\s+/g, ' ');
}

function normalizeGroupDeliveryPin(value) {
  const pin = String(value || '').trim();
  return /^\d{4,6}$/.test(pin) ? pin : '';
}

function normalizeGroupDeliveryUserStatus(value) {
...
function normalizeGroupDeliveryCycleStatus(value) {
  const normalized = String(value || '').trim().toLowerCase();
  return normalized === 'submitted' ? 'submitted' : 'open';
}

function normalizeGroupDeliveryEntryStatus(value) {
  const normalized = String(value || '').trim().toLowerCase();
...
function normalizeCelaviveSurveyPayload(body) {
  const futureActivities = normalizeCelaviveMultiChoice(
    body && body.futureActivities,
    CELAVIVE_SURVEY_ACTIVITY_OPTIONS,
    'Choose at least one future activity.'
  );
  const payload = {
    fullName: String(body && body.fullName ? body.fullName : '').trim(),
    mobileNumber: normalizePhilippineMobile(body && body.mobileNumber ? body.mobileNumber : ''),
    ...
    profession: String(body && body.profession ? body.profession : '').trim(),
    invitedBy: String(body && body.invitedBy ? body.invitedBy : '').trim(),
    partyRating: normalizeCelaviveRating(body && body.partyRating, 'Choose your Celavive Spa Party rating.'),
    futureActivities,
    activityWillingness: normalizeCelaviveActivityWillingness(body && body.activityWillingness, futureActivities)
  };
...
function normalizeCelaviveRating(value, message) {
...
function normalizeCelaviveActivityWillingness(value, activities) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw badRequest('Rate your willingness to join each selected future activity.');
  }

  return Object.fromEntries(activities.map((activity) => {
    const rating = normalizeCelaviveRating(value[activity], `Rate your willingness to join ${activity}.`);
...
function getMailer() {
  if (!mailer) {
    mailer = createMailer({ getEnv, createError });
  }
...
function isEmailConfigured() {
  return getMailer().isMailConfigured();
}

// Email is a side effect of saving something; a mail failure must never lose the
// record that was just written, so the outcome is reported rather than thrown.
async function trySendEmail({ to, subject, html, attachments, context }) {
  if (!isEmailConfigured()) {
    console.warn(`[email] Skipped "${subject}" to ${to}: GMAIL_USER / GMAIL_APP_PASSWORD are not set.`);
    return { sent: false, reason: 'not-configured' };
  }

  try {
    await getMailer().sendMail({ to, subject, html, attachments });
    return { sent: true };
  } catch (error) {
    console.error(`[email] Failed to send "${subject}" to ${to} (${context || 'general'}): ${error.message}`);
    return { sent: false, reason: 'send-failed', message: error.message };
  }
}
...
async function readSheetObjects(sheetName, options = {}) {
  return mapSheetRowsToObjects(await readSheetRows(sheetName, options));
}

function mapSheetRowsToObjects(rows) {
  if (rows.length === 0) {
    return [];
  }
  ...
  // Number every row before dropping the blank ones so __rowNumber keeps pointing
  // at the real storage row, which updates and deletes are addressed by.
  return dataRows
    .map((row, index) => ({ row, rowNumber: index + 2 }))
    .filter((entry) => entry.row.some((cell) => String(cell || '').trim() !== ''))
    .map((entry) => mapRowToObject(headers, entry.row, entry.rowNumber));
}
...
function readServiceAccount(purpose = '') {
  ...
  const serviceAccountFile = shouldUseFirebaseCredentials
    ...
    : env.googleServiceAccountFile;
  const inlineServiceAccount = shouldUseFirebaseCredentials
    ? buildInlineServiceAccount({
        projectId: env.firebaseProjectId || env.googleCloudProjectId,
    ...
    : buildInlineServiceAccount({
        ...
        privateKeyId: env.googlePrivateKeyId
      });
...
function buildInlineServiceAccount({ projectId, clientEmail, privateKey, privateKeyId }) {
...
module.exports = {
  ...
  __test: {
    ...
    decorateEvent,
    getDefaultRsvpAccepting,
    isFirebaseConfigured,
    mapSheetRowsToObjects,
    buildSpecialFormColumns,
    evaluateSpecialFormVisibility,
    formatConfirmationTimestamp,
    isSpecialEvent,
    normalizeCelaviveSurveyPayload,

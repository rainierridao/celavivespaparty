...
const EVENT_TYPES = ['OPP', 'Celavive Spa Party', BEAUTY_CARAVAN_EVENT_TYPE, CELAVIVE_RAFFLE_EVENT_TYPE, INBODY_EVENT_TYPE, WELLNESS_QUIZ_EVENT_TYPE, GROUP_DELIVERY_EVENT_TYPE];
...
async function handleApiRequest({ method, path: requestPath, headers = {}, body, protocol = 'https' }) {
  ...
  if (method === 'GET' && publicRsvpResponsesMatch) {
    ...
    // Only expose RSVP responses publicly for Celavive Spa Party events
    if (String(event.eventType || '').trim() !== 'Celavive Spa Party') {
...
function matchesEventIdentifier(row, identifier) {
  ...
  // Allow a trailing "view" suffix only for Celavive Spa Party public slugs
  const allowViewSuffix = String(event.eventType || '').trim() === 'Celavive Spa Party';

...
// Deleted rows leave gaps in the stored row numbers, so keep every row at its own
// row number position. Callers address rows by number when updating or deleting,
// and a compacted list would point them at the wrong document.
function alignRowsByRowNumber(entries) {
  ...
  const rows = new Array(ordered[ordered.length - 1].rowNumber).fill(null);

  ordered.forEach((entry) => {
    rows[entry.rowNumber - 1] = entry.values;
  });

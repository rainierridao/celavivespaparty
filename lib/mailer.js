...
function createMailer({ getEnv, createError }) {
  ...
  function deliver({ config, to, message }) {
    return new Promise((resolve, reject) => {
      ...
      let buffer = '';
      let settled = false;
      // `send` is the command for this step; `expect` is the reply code the server
      // must answer THAT command with. Step 0 sends nothing and just waits for the
      // opening greeting.
      const steps = [
      ...
      socket.on('data', (chunk) => {
        ...
        while (true) {
          ...
          // QUIT is the last command. The message is already accepted at this
          // point, so end the stream and wait for the write to flush rather than
          // tearing the socket down mid-write.
          if (nextStep.send === 'QUIT') {
            socket.end(`${nextStep.send}\r\n`, () => finish());
            return;
          }

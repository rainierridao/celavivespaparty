...
const WELLNESS_RAFFLE_WIN_CHANCE = 40;
...
function normalizeWellnessRaffleConfig(value) {
  ...
  const prizes = rawPrizes.map((prize, index) => {
    ...
    ids.add(id);
    return { id, label, color, quantity, chance: Math.round(chance * 100) / 100 };
  });
  ...
  if (prizes.length) {
    ...
    if (Math.abs(totalChance - WELLNESS_RAFFLE_DISTRIBUTION_TOTAL) > 0.001) {
      throw badRequest('Winning prize shares must total exactly 100% of the 40% winning pool.');
    }
  }

  return { enabled, title, losingLabel, losingColor, prizes };
}
...
function parseWellnessRaffleConfig(value) {
  ...
  try {
    ...
    if (prizes.length && Math.abs(legacyTotal - WELLNESS_RAFFLE_WIN_CHANCE) < 0.001) {
      parsed.prizes = prizes.map((prize) => ({
        ...prize,
        chance: Math.round(Number(prize.chance) * 250) / 100
      }));
    }
